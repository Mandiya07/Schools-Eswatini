...
const MOCK_BEHAVIOR_HISTORY = [
  { id: 'bh1', type: 'merit', points: 10, reason: 'Exceptional teamwork in Science project', date: '2026-04-12' },
  { id: 'bh2', type: 'demerit', points: -5, reason: 'Late to class (3rd time)', date: '2026-04-08' },
  { id: 'bh3', type: 'merit', points: 5, reason: 'Helping a junior student with reading', date: '2026-04-02' }
];
...
const MOCK_ATTENDANCE_SUMMARY = {
  ...
  history: [
    ...
    { date: '2026-04-12', status: 'absent', reason: 'Medical Duty' }
  ]
};

const MOCK_TRANSCRIPTS = [
  { id: 't1', semester: 'Year 1, Sem 1', gpa: '3.72', status: 'Released', date: '2025-12-20' },
  { id: 't2', semester: 'Year 1, Sem 2', gpa: '3.85', status: 'Pending Finance Clearance', date: 'Estimated: 2026-06-15' }
];
      ...
      <div className="flex justify-between items-end mb-12">
        ...
        <div className="flex items-center gap-4">
          <select 
            ...
            className="bg-white border border-slate-200 text-slate-700 text-xs font-bold rounded-xl px-4 py-2 focus:ring-2 focus:ring-blue-500"
          >
            <option value="">Default (Primary/High School)</option>
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          {institutionType === 'Tertiary' ? (
            <div className="w-full mb-4">
              <div className="bg-amber-50 border border-amber-200 text-amber-800 p-6 rounded-2xl flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-black text-lg mb-1">Financial Sponsor Portal</h3>
                  <p className="font-medium text-sm">
                    As a sponsor of a tertiary student, your access is limited to financial ledgers and official transcripts (if permission is granted by the student). Daily attendance, behavior alerts, and PTA features are disabled for adult students.
                  </p>
                </div>
              </div>
            </div>
          ...
          {institutionType !== 'Tertiary' && (
           ...
           <button 
             ...
             className={`px-6 py-4 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all ${(activeView === 'finance' || institutionType === 'Tertiary') ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-600/20' : 'bg-white text-slate-500 hover:bg-slate-100 border border-slate-200'}`}
           >
                  ...
                  <section>
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                        {institutionType === 'Tertiary' 
                          ? 'Module Performance & Transcripts' 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <section>
                      <div className="flex items-center gap-4 mb-6">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                          {institutionType === 'Primary' ? 'Behavior & Social Star Log' : 'Behavior & Discipline'}
                        </h3>
                      </div>
                    ...
                    {/* Merits & Demerits Section (Primary/High Only) */}
                    {institutionType !== 'Tertiary' && (
                      <section className="md:col-span-2 bg-slate-50 rounded-[40px] p-8 md:p-10 border border-slate-200">
                        <div className="flex justify-between items-center mb-8">
                          <h3 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                            <Sparkles className="w-8 h-8 text-amber-500" /> merit & behavior log
                          </h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                           {MOCK_BEHAVIOR_HISTORY.map(item => (
                             <div key={item.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                               <div className="flex justify-between items-start mb-4">
                                  <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${item.type === 'merit' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                    {item.type} {item.points > 0 ? `+${item.points}` : item.points}
                                  </span>
                                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.date}</span>
                               </div>
                               <p className="text-sm font-bold text-slate-700 leading-relaxed mb-1">{item.reason}</p>
                             </div>
                           ))}
                        </div>
                      </section>
                    ...
                    {/* Homework & Assignments (Primary/High Only) */}
                    {institutionType !== 'Tertiary' && (
                      <section className="md:col-span-2">
                        <div className="flex items-center gap-4 mb-6">
                          ...
                          <h3 className="text-2xl font-black text-slate-900 tracking-tight">Active Homework & Tasks</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {[
                            { title: 'Division Exercises', subject: 'Mathematics', due: 'Tomorrow', status: 'pending' },
                            { title: 'Animal Habitat Poster', subject: 'Science', due: 'Friday', status: 'submitted' },
                            { title: 'SiSwati Oral Reading', subject: 'Languages', due: 'Monday', status: 'pending' }
                          ].map((task, i) => (
                             <div key={i} className={`p-6 rounded-3xl border ${task.status === 'submitted' ? 'bg-emerald-50 border-emerald-100' : 'bg-white border-slate-100 shadow-sm'}`}>
                                <div className="flex justify-between items-start mb-4">
                                   <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest">{task.subject}</span>
                                   {task.status === 'submitted' && <CheckCircle className="w-4 h-4 text-emerald-500" />}
                                </div>
                                <h4 className="font-bold text-slate-900 mb-2">{task.title}</h4>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Due: {task.due}</p>
                             </div>
                          ))}
                        </div>
                      </section>
                        ...
                        <div className="flex flex-col lg:flex-row gap-10 items-center relative z-10">
                          <div className="flex-1">
                            <h3 className="text-2xl font-black mb-4 tracking-tight flex items-center gap-3">
                              <Users className="w-6 h-6 text-blue-400" /> Direct Teacher Messaging
                            </h3>
                            <p className="text-slate-400 font-medium leading-relaxed mb-8 max-w-lg">
                              Have questions about {data.studentName}'s homework or behavior? Message their teachers directly for quick updates.
                            </p>
                            <div className="flex flex-wrap gap-4">
                              <button className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-blue-600/20 transition-all flex items-center gap-3">
                                Start New Chat
                              </button>
                              <button className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-black uppercase tracking-widest text-[10px] transition-all">
                                View Chat History
                              </button>
                            </div>
                          </div>
                          <div className="w-full lg:w-72 shrink-0 space-y-3">
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 px-2">Class Teachers</p>
                            {['Mr. Zwane (Math)', 'Ms. Khoza (Science)', 'Dr. Nxumalo (SiSwati)'].map(teacher => (
                              <div key={teacher} className="p-4 bg-white/5 border border-white/10 rounded-2xl flex justify-between items-center group cursor-pointer hover:bg-white/10 transition-colors">
                                <span className="text-[11px] font-bold text-slate-200">{teacher}</span>
                ...
                <div className="lg:col-span-1 space-y-8">
                  <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm p-8">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                       <GraduationCap className="w-4 h-4 text-indigo-600" /> Academic Transcripts
                    </h3>
                    <div className="space-y-4">
                       {MOCK_TRANSCRIPTS.map(t => (
                         <div key={t.id} className="p-5 bg-slate-50 border border-slate-100 rounded-2xl group hover:border-indigo-300 transition-all">
                            <h4 className="font-bold text-slate-900 text-sm mb-1">{t.semester}</h4>
                            <div className="flex justify-between items-center mb-3">
                               <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">GPA: {t.gpa}</p>
                               <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${t.status === 'Released' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                                 {t.status}
                               </span>
                            </div>
                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{t.date}</p>
                            {t.status === 'Released' && (
                              <button className="w-full mt-4 py-2 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2">
                                <Download className="w-3 h-3" /> Download Official PDF
                              </button>
                            )}
                         </div>
                       ))}
