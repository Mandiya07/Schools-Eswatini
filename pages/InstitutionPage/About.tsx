    ...
    <div className="space-y-32">
      <section className="space-y-10">
        ...
        <p className="text-2xl text-slate-500 font-medium leading-relaxed max-w-3xl">
          {typeof about.overview === 'object' && about.overview !== null
            ? (about.overview as any).description || (about.overview as any).title || ''
            : about.overview || ''}
        </p>
      </section>
      ...
      <section className="space-y-16">
        ...
        <MissionSection mission={about.mission} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">The Vision</h3>
        <VisionSection vision={about.vision} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Core Values</h3>
        <CoreValuesSection values={about.coreValues} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Leadership Profile</h3>
        <LeadershipSection leadership={about.leadership} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Accreditation & Compliance</h3>
        <AccreditationSection accreditation={about.accreditation} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Institutional Statistics</h3>
        <StatisticsPanel statistics={about.statistics} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Campus Overview</h3>
        <CampusOverview facilities={about.facilities} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Community Impact</h3>
        <CommunityImpact community={about.community} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Download Center</h3>
        <DownloadCenter downloads={about.downloads} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>

      <section className="space-y-16">
        <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Our Journey</h3>
        <HistorySection foundingBackground={about.foundingBackground} history={about.history} primaryColor={(inst.theme?.primaryColor || '#2563eb')} />
      </section>
    </div>
  );
};
