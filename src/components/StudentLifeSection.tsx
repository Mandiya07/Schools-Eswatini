          ...
          {life.clubs.map((club, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-8 gap-4">
                <span className="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-full text-[9px] font-black uppercase tracking-widest break-words min-w-0">
                  {club.focus}
                </span>
                <ChevronRight className="shrink-0 w-5 h-5 text-slate-300 group-hover:text-indigo-600 transition-colors" />
              </div>
              <h4 className="text-2xl font-black text-slate-900 mb-4 tracking-tight leading-snug break-words">{club.name}</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {life.facilities.map((fac, idx) => (
              <div key={idx} className="p-8 bg-slate-50 rounded-3xl border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
                <h5 className="text-sm font-black text-slate-900 uppercase tracking-widest mb-2 leading-snug break-words">{fac.name}</h5>
